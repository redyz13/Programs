    ...
    <body id="page-top">
        ...
        <section class="bg-light">
                ...
                <div class="card" style="margin-top: 10px;">
                    ...
                    <div class="d-flex justify-content-between flex-column card-details pt-2">
                        <% ResultSet rs = statement.executeQuery("SELECT numero FROM carta c INNER JOIN titolare t ON c.codiceTitolare = t.codiceTitolare WHERE t.username = \""+ username +"\""); 
                            while(rs.next()) {
                        %>
                        <% out.println("<b><p>"+rs.getString(1)+"</p></b>"); %>
                        <% } %>
                        <% rs.close(); %>
                        ...
                        <%  rs = statement.executeQuery("SELECT dataScadenza FROM carta c INNER JOIN titolare t ON c.codiceTitolare = t.codiceTitolare WHERE t.username = \""+ username +"\""); 
                            while(rs.next()) {
                        %>
                        <% out.println("<b><p>"+rs.getDate(1)+"</p></b>"); %>
                        <% } %>
                        <% rs.close(); %>
                        </div>
                    ...
                    <div class="d-flex position-relative">
                        ...
                        <%  rs = statement.executeQuery("SELECT ccv FROM carta c INNER JOIN titolare t ON c.codiceTitolare = t.codiceTitolare WHERE t.username = \""+ username +"\""); 
                            while(rs.next()) {
                        %>
                        <% out.println("<b><p>"+rs.getInt(1)+"</p></b>"); %> 
                        <% } %>
                        <% rs.close(); %>   
                    </div>
            ...
            <table class="table">
                ...
                <tbody>
                <%
                    rs = statement.executeQuery("SELECT codiceMovimento, importoTransazione, dataTransazione, descrizione FROM movimenti m INNER JOIN carta c ON c.codiceCarta = m.codiceCarta INNER JOIN titolare t ON c.codiceTitolare = t.codiceTitolare WHERE t.username = \""+ username +"\"");
                    while(rs.next()) {
                %>
                    <tr>
        ...
        <footer class="py-5 bg-dark">
            <div class="container"><p class="m-0 text-center text-white">Antonio D'Auria &copy; Elaborato Esame</p></div>
        </footer>
        <!-- Bootstrap core JS-->
